...
const StarRating = ( { total = 5, rating } ) => {
	const safeRating = Math.min( Math.max( Number( rating ) || 0, 0 ), total );
	const normalized = Math.round( safeRating * 10 ) / 10;
	const filled = Math.floor( normalized );
	const fraction = Math.round( ( normalized - filled ) * 10 ) / 10;
	const empty = total - Math.ceil( normalized );
